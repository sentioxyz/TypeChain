...
export function generateEventFilters(events: EventDeclaration[]) {
  if (events.length === 1) {
    ...
    const typedEventFilter = `${generateEventIdentifier(event, { includeArgTypes: false })}`
    ...
    return events
      .map((event) => {
        const typedEventFilter = `${generateEventIdentifier(event, { includeArgTypes: true })}`
...
export const EVENT_IMPORTS = ['TypedEventFilter', 'TypedEvent',
  // 'OnEvent'
]
